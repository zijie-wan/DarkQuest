...
int AnaModule::process_event(PHCompositeNode* topNode)
{
  ...
  nHits = 0;
  if (hitVector) {
    for (int k = 0; k < (int)hitVector->size(); ++k) {
        SQHit* h = hitVector->at(k);
        detectorID[nHits]    = h->get_detector_id();
        elementID[nHits]     = h->get_element_id();
        pos[nHits]           = h->get_pos();
        tdcTime[nHits]       = h->get_tdc_time();
        driftDistance[nHits] = h->get_drift_distance();
        ++nHits;
        if (nHits >= 15000) break;
    }
  }
  ...
  int trigBits = sqEvent->get_trigger();
  bool pass_trigger = (trigBits > 0 &&
    ((trigBits & (nim1TriggerMask|nim3TriggerMask)) != 0));

  if (pass_trigger) saveTree->Fill();
  }
...
int AnaModule::GetNodes(PHCompositeNode* topNode)
{
  ...
  if (!sqEvent) {
    std::cout << "SQEvent not found" << std::endl;
  ...
  if (saveReco && !recEvent) {
    std::cout << "SRecEvent not found" << std::endl;
    return Fun4AllReturnCodes::ABORTEVENT;
  }
